...
int main() {
	...
	struct Client client = initializare(2, 40, "Popescu", 2000, 'G');
	afisare(client);
	...
	clienti[0] = initializare(1,30, "Ionescu", 350, 'L');
	clienti[1] = initializare(2, 44, "Antonescu", 500, 'C');
	clienti[2] = initializare(3, 35, "Florescu", 200, 'B');

	printf("Clientul cautat:\n");
	struct Client c = getPrimulClientDupaNume(clienti, nrClienti, "Florescu");
	afisare(c);
	free(c.nume);
	printf("Vector:\n");
	...
	copiazaClientiCuBugetMare(clienti, nrClienti, 200, &clientiCuBugetMare, &nrClientiCuBugetMare);
